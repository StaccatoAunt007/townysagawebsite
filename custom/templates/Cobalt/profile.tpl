        ...
        <div class="{if count($WIDGETS_LEFT) && count($WIDGETS_RIGHT)}col-lg-6{elseif count($WIDGETS_LEFT) || count($WIDGETS_RIGHT)}col-lg-9{else}col-lg-12{/if}">
            ...
            {nocache}
                {if isset($LOGGED_IN)}
                    ...
                    {else}
                        <div class="btn-group">
                            <a href="{$SETTINGS_LINK}" class="btn btn-secondary"><i class="fa-solid fa-gear"></i></a>
            ...
            {/nocache}
                ...
                <div class="profile-header-bottom">
                    ...
                    <li class="nav-item">
                        <a class="profile-nav-link" id="{$key}-tab" data-toggle="tab" href="#{$key}" role="tab" aria-controls="{$key}" aria-selected="false">
                            {if $key == "friends"}
                            ...
                            {/if}
                            <span class="web_only"> {$tab.title}</span>
                        </a>
                    </li>
                    {/foreach}
                </ul>
                ...
                <div class="card-body main-card-body">
                    <div class="tab-content" id="myTabContent">
                                ...
                                <article class="panel panel-theme" id="post-id-{$post.id}">
                                    ...
                                    <div class="panel-footer">
                                        <a href="{if $post.reactions_link ne "#"}{$post.reactions_link}{else}#{/if}" class="grey-link" data-content='{if isset($post.reactions.reactions)} {foreach from=$post.reactions.reactions item=reaction name=reactions}<a href="{$reaction.profile}" style="{$reaction.style}"><img class="avatar-img" src="{$reaction.avatar}" alt="{$reaction.username}" style="max-height:30px; max-width:30px;" /> {$reaction.nickname}</a>{if !$smarty.foreach.reactions.last}<br />{/if}{/foreach} {else}{$post.reactions.count}{/if}'>
                                            <i class="fa-solid fa-thumbs-up"></i> {$post.reactions.count|regex_replace:'/[^0-9]+/':''}  </a>
                                        ...
                                        <span class="float-right">
                                            ...
                                            <a class="grey-link" href="#" data-toggle="modal" data-target="#editModal{$post.id}"><i class="fa-solid fa-pencil"></i> <span class="web_only">{$EDIT}</span></a>
                                            <a class="grey-link" href="#" onclick="deletePost({$post.id})"><i class="fa-solid fa-trash"></i> <span class="web_only">{$DELETE}</span></a>
                                            {/if}
                                        </span>
                                    </div>
                                </article>
                        ...
                        <div class="row">
                            <div class="col-md-4 justify-content-center">
                                <div class="profile-about-box-new">
                                    ...
                                    <div class="text">
                                        <div>{$ABOUT_FIELDS.registered.title}</div>
                                        <span rel="tooltip" title="{$ABOUT_FIELDS.registered.tooltip}">{$ABOUT_FIELDS.registered.value}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="profile-about-box-new">
                                    ...
                                    <div class="text">
                                        <div>{$ABOUT_FIELDS.last_seen.title}</div>
                                        <span rel="tooltip" title="{$ABOUT_FIELDS.last_seen.tooltip}">{$ABOUT_FIELDS.last_seen.value}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="profile-about-box-new">
                                    ...
                                    <div class="text">
                                        <div>{$ABOUT_FIELDS.profile_views.title}</div>
                                        <span class="about-title">{$ABOUT_FIELDS.profile_views.value}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div style="color: #FFF; font-size: 1.2rem; font-weight: 500; margin-bottom: .75rem">{$ABOUT}</div>
                            ...
                            {if is_numeric($key)}
                                <span style="font-weight: 500">{$field.title}:</span> {$field.value}<br />
                            {/if}
                            {/foreach}
                            {else}
