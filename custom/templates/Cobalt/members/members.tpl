...
<div class="container">
    ...
    <div class="row">
        <div class="col-lg-3">
            ...
            <div class="user-nav">
                <a href="{$MEMBER_LIST_URL}"
                    class="user-nav-link{if $VIEWING_LIST eq "overview"} active{/if}">{$OVERVIEW}</a>
                {foreach from=$SIDEBAR_MEMBER_LISTS item=list}
                    <a href="{$list->url()}" class="user-nav-link{if $VIEWING_LIST eq $list->getName()} active{/if}"><i
                            class="{if $list->getIcon()}{$list->getIcon()}{else}dot circle icon{/if}"></i>
                        {$list->getFriendlyName()}</a>
                {/foreach}
            </div>
                    ...
                    <div class="card-body">
                        <select class="form-control" onchange="viewGroup(this)">
                            ...
                            {foreach from=$GROUPS item=group}
                                <option value="{$group.id}" {if $VIEWING_GROUP.id == $group.id} selected {/if}>{$group.name}
                                </option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    <div class="row" id="new-members-grid" style="margin-right: -10px; margin-left: -10px">
                        {foreach from=$NEW_MEMBERS_VALUE item=member}
                            <div class="col-3" style="padding-right: 10px; padding-left: 10px; ">
                                <a class="d-block mb-3" href="{$member->getProfileUrl()}" data-toggle="popup"
                                    data-poload="{$USER_INFO_URL}{$member->data()->id}">
                                    <img class="avatar-img w-100" src="{$member->getAvatar()}"
        ...
        {if $VIEWING_LIST == "group" || $MEMBER_LISTS_VIEWING|count}
            ...
            {else}
                ...
                {foreach from=$MEMBER_LISTS_VIEWING item=list}
                    <div class="{if $VIEWING_LIST == "overview"}col-lg-4{else}col-lg-12{/if} members-col">
                        <div class="card members-card">
                            <div class="card-header header-theme">{$list->getFriendlyName()}</div>
                            <div class="card-body">
                                <div style="margin-bottom: 2rem" id="member_list_{$list->getName()}">
                                </div>
