...
<div class="f-small">
    <div class="container d-flex">
        <div class="f-copyright">
            <div class="f-top-copy"><span>{$SITE_NAME} <i class="fa-regular fa-copyright"></i> {'Y'|date}.</span> All rights reserved.</div>
            <div class="f-bottom-copy">{$THEME_FOOTER_DISCLAIMER}</div>
        </div>
        {if isset($AUTO_LANGUAGE) || ($SOCIAL_MEDIA_ICONS|count > 0) || $PAGE_LOAD_TIME}
        <div class="f-icons">
            ...
            {if $SOCIAL_MEDIA_ICONS|count > 0}
                ...
                <a href="{$icon.link}" target="_blank"><i id="social-{$icon.short}" class="fa-brands fa-{$icon.long}"></i></a>
                {/foreach}
            {/if}
        </div>
        {/if}
        <div class="f-tc">
            {if isset($THEME_FOOTER_SIZE) && $THEME_FOOTER_SIZE !== 'large'}
            <a class="f-link terms-link" href="{$TERMS_LINK}">{$TERMS_TEXT}</a><br />
            <a class="f-link privacy-policy-link" href="{$PRIVACY_LINK}">{$PRIVACY_TEXT}</a>
            {/if}
        </div>
        <div class="f-icon-col">
            <a href="{$THEME_COLDFIRE_URL}" rel="noopener" target="_blank">
                <picture>
                    ...
                    <img class="f-credit-img" alt="Coldfire Design" src='{$THEME_COLDFIRE_CREDIT_IMG}' />
                </picture>
            </a>
        </div>
    </div>
...
autoLanguage.setAttribute('data-original-title', '{$AUTO_LANGUAGE_TEXT} (' + (autoLanguageValue === 'true' ? '{$ENABLED}' : '{$DISABLED}') + ')');
